...
wildtype_sim <- load_sim_data("data/simulated/run_050217_wt_counts.tsv")
...
ribo_data <- read_tsv("data/simulated/run_050217_wt_density.tsv", col_names = c("time", "gene", "ribosomes", "transcripts", "density"))
...
# 
# ggplot(all_sim %>% filter(!is.na(class)), aes(x=factor(gene_number, levels=mixedorder(gene_number)), y=count, fill=factor(class))) +
#   geom_bar(stat="identity") +
