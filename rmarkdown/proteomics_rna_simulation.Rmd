...
protein_rna_plot <- ggplot(proteomics_rna_9, aes(x = area_norm, y = avg_tpm, color=strain)) + 
  ...
  geom_text_repel(data=proteomics_rna_9 %>% mutate(gene_number.x = if_else(strain == "atten", "", gene_number.x)), 
                  aes(area_norm, avg_tpm, label = gene_number.x),
...
cor_plot <- ggplot(sim_data_cors, aes(x = time, y = cor, color = weight, group = weight)) + 
  geom_line() + 
  ylim(-1,1) + 
...
cor_plot <- ggplot(sim_data_cors, aes(x = time, y = cor, color = condition, group = condition)) + 
  geom_line() + 
  ylim(-1,1) + 
  geom_vline(xintercept = 1200) +
  scale_color_manual(values = cbPalette)
