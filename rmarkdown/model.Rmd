...
We first assume a polycistronic transcript with two genes $a$ and $b$. For the effective translation initiation rate $b_in$ of gene $b$, we define the following relationship
...
initiation. We assume that if the initiation rate ever exceeds the elongation rate, ribosomes would quickly back up on the transcript and block initiation. Thus in our model, the translation initiation rate can never exceed the elongation rate. 

We define the aggregate translation rate
$$i_b = \min\left\{b_\text{reinit} + b_\text{de novo}, k\right\}$$
where $k$ is the maximum initiation rate in the absence of any translational coupling effects, $b_\text{reinit}$ is the rate of upstream translating ribosomes reinitiating on gene $b$ and $b_\text{de novo}$ is the rate of ribosomes initiating \textit{de novo} on gene $b$. We assume that together $b_\text{reinit}$ and $b_\text{de novo}$ can never exceed the maximum initiation rate $k$.

Lastly we define reinitiation and de novo initiation rates on gene $b$ as follows

$$b_\text{reinit} = q\dot{A}$$
$$b_\text{de novo} = z\dot{A} + w$$
Where $\dot{A}$ is the rate of ribosomes flowing from the end of upstream gene $a$, and $q$ ranges from 0 to 1 and represents the proportion of that ribosome flow reinitiating on gene $b$. We also assume that the rate of \textit{de novo} initation depends, in part, on upstream ribosomes relaxing secondary structure around the ribosome binding site of gene $b$. Thus, the rate of \textit{de novo} initiation is given the upstream ribosome flow $\dot{A}$ scaled by some constant $z$, and \textit{de novo} initiation rate $w$ that does not depend on upstream ribosome flow.

We can simplify the effective initiation rate to
$$b_\text{in} = \min\left\{q\dot{A} + z\dot{A} + w, \tau_b, k\right\}$$
<!--
## General case
