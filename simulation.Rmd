...
```{r load_sim_data, echo=FALSE, warning=FALSE, message=FALSE}

wildtype_sim <- load_sim_data("data/simulated/run_033117.out.txt")
...
```{r load_exp_data, echo=FALSE, warning=FALSE, message=FALSE}
...
```{r late_sim_exp_plot, fig.width=15}
...
```{r all_exp_plot, fig.height=9, fig.width=15}

all_exp_plot <- ggplot(mean_exp %>% filter(!is.na(class)), aes(x=factor(gene_number, levels=mixedsort(gene_number)), y=area_norm, fill=factor(class))) +
  geom_bar(stat="identity") +
  facet_wrap(~time, ncol = 1, scales="free_x") +
...
```{r all_exp_scaled_plot, fig.height=9, fig.width=15}

all_exp_plot + facet_wrap(~time, ncol = 1, scales="free")
