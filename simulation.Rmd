...
allgenes <- filter(all_data, strain == 'wt', time == '9min')

ggplot(allgenes, aes(x=fct_reorder(gene_number, class), y=area_norm, fill=factor(class))) +
  stat_summary(geom="bar", fun.y = "mean")
```
...
allgenes_sim <- filter(wildtype, time == 1200) %>% rename(sim_time = time)

allgenes_mean <- group_by(allgenes, gene_number, class) %>% summarize(area_norm = mean(area_norm))

allgenes_sim_exp <- full_join(allgenes_mean, allgenes_sim) %>% 
...
ggplot(allgenes_sim_exp, aes(x=factor(gene_number, levels=mixedsort(gene_number)), y=count, fill=factor(class))) +
  geom_bar(stat = "identity") +
  facet_wrap(~source, scales = "free", ncol = 1, strip.position="left") +
...
ggplot(allgenes_1min, aes(x=fct_reorder(gene_number, class, fun = ), y=area_norm, fill=factor(class))) +
  stat_summary(geom="bar", fun.y = "mean")
