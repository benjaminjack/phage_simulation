...
```{r}
data <- read_tsv("../data/simulation/three_genes/three_genes_rnase_prom_counts.tsv")
data %>% filter(species %in% c("proteinX", "proteinY", "proteinZ")) -> data
line_plot <- ggplot(data, aes(x = time, y = transcript, color=species)) + 
  geom_line() + 
  theme(legend.position="none")
bar_plot_late <- ggplot(data %>% filter(time > 499), aes(fill=species, x=species, y=transcript)) +
  geom_bar(stat="identity") + 
  scale_y_continuous(expand=c(0,0))
bar_plot_early <- ggplot(data %>% filter(time > 100, time < 101), aes(fill=species, x=species, y=transcript)) + 
  geom_bar(stat="identity") + 
  theme(legend.position="none") + 
  scale_y_continuous(expand=c(0,0))
...
```{r}
data <- read_tsv("../data/simulation/three_genes/three_genes_prom_deg_counts.tsv")
data %>% filter(species %in% c("proteinX", "proteinY", "proteinZ")) -> data
line_plot <- ggplot(data, aes(x = time, y = transcript, color=species)) + 
  geom_line() + 
  theme(legend.position="none")
