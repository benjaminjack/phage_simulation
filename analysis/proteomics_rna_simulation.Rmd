...
load_rna_remapped <- function(path) {
  labels <- read_csv("../../data/id_map.csv") %>% mutate(accession = trimws(accession))

  rna <- read_csv(path) %>%
    # Replace strain names with something more useful
    mutate(strain = str_replace_all(strain, c("wildtype" = "wt"))) %>%
    mutate(tpm = tpm/1e6) %>%
    group_by(b_rep, time, strain) %>%
    group_by(strain, time, b_rep) %>%
    do(full_join(., labels, by = c("gene_number" = "gene_number")) %>% fill(strain, time, b_rep)) %>%
    mutate(prot_id = accession) %>%
    ungroup() %>%
    mutate(gene_number = str_replace_all(gene_number, c("10A" = "10"))) %>%
    group_by(time, strain, gene_number, prot_id) %>%
    summarize(avg_tpm = mean(tpm))
...
rna <- load_rna_remapped("../../data/transcripts_remapped.csv")
...
proteomics_rna_9 <- filter(proteomics_rna, time %in% c(5, 9), !is.na(class), avg_tpm > 0, area_norm > 0) %>% 
  group_by(strain) %>%
...
sim_data_wt <- data_frame(path = paste0("T7_082317_strong3_5_w", seq(1,15)), weight = seq(1, 15)) %>%
  mutate(data = map(path, ~load_simulation(paste0("../../runs/", .)))) %>%
  mutate(condition = "wt") %>%
  unnest()

sim_data_atten <- data_frame(path = paste0("T7_082317_strong3_5_w", seq(1,15), "_r"), weight = seq(1, 15)) %>%
  mutate(data = map(path, ~load_simulation(paste0("../../runs/", .)))) %>%
...
bar_sim <- filter(sim_data, time == 1200, !is.na(gene_number))
...
bar_sim_exp <- inner_join(phage_rna %>% select(-time), bar_sim %>% select(-time) %>% filter(weight == 1, condition == "wt"), by = c("prot_id" = "accession", "gene_number" = "gene_number"))

cor.test(bar_sim_exp$avg_tpm, bar_sim_exp$transcripts, method = "spearman")
...
protein_rna_sim_scatter_wt <- ggplot(sim_data %>% filter(time == 1200, gene_number != 1, weight == 3, condition=="wt"), 
       aes(y = log10(proteins), x = log10(transcripts))) +
  geom_point() +
#  xlim(1, 3.5) + ylim(2, 3.8) +
  scale_color_manual(values = cbPalette)

protein_rna_sim_scatter_wt_class <- ggplot(sim_data %>% filter(time == 1200, gene_number != 1, weight == 3, condition=="wt"), 
...
protein_rna_sim_scatter_labeled <- protein_rna_sim_scatter + 
  geom_text_repel(data=sim_data %>% filter(time == 1200, gene_number != 1, weight == 3) %>% mutate(gene_number = if_else(condition == "atten", "", gene_number)), 
...
# if(SAVE_PLOTS) { save_plot("../../figures/protein_rna_sim_scatter.pdf", protein_rna_sim_scatter_wt_class, base_aspect_ratio = 1.5) } 
...
sim_data_3 <- filter(sim_data, weight == 3, condition == "wt", !is.na(gene_number)) # %>% bind_rows(sim_data %>% filter(gene == "bound_ribosome", weight == 3, condition == "wt") %>% mutate(class = 0))

line_plot <- ggplot(sim_data_3, aes(x = time, y = proteins, group = factor(gene), color = factor(class))) +
  geom_line() + xlim(0, 800) + ylim(0, 4000) +
  scale_color_manual("class", values = cbPalette[c(3,4,7)]) +
  theme(legend.position = c(0.1, 0.85))

save_plot("../../figures/line_plot.pdf", line_plot, base_aspect_ratio = 1.4)
