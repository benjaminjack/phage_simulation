...
bar_sim_plot <- ggplot(bar_sim %>% filter(time == 1500), aes(x=factor(gene_number, unique(mixedsort(gene_number))), y=transcript, fill=factor(class))) +
  ...
  scale_x_discrete(breaks = gene_labels[c(T, rep(F, 2))])
...
bar_sim_zoom <- bar_sim_plot + coord_cartesian(ylim = c(0, 400), xlim = c(5, 31.9))

if (SAVE_FIGS) {
  save_plot("./figures/T7_rna_exp_sim_degrade_zoom.pdf", bar_sim_zoom, base_width = 6, base_height = 3)
}
```
...
exp_cor <- ggplot(proteomics_rna %>% filter(time != 1), aes(x = log10(avg_tpm), y = log10(area_norm), color = factor(class))) + 
  geom_point() + 
  xlab("log10(transcript abundance)") + 
  ylab("log10(protein abundance)") + 
  scale_color_discrete(name = "class") + facet_wrap(~time, scales = "free", labeller = as_labeller(time_labels)) +
  stat_ellipse() + 
