...
rna_no_ribo <- ggplot(phagerna2, aes(x = strain, y = tpm, fill = factor(b_rep))) + 
  geom_bar(stat = "identity", position = "dodge") + 
  facet_grid(org ~ time)

rna_no_ribo

save_plot("figures/rna_no_ribo.pdf", rna_no_ribo, base_aspect_ratio = 2)
```
...
gp10_plot <- ggplot(phagerna2, aes(x = strain, y = tpm, fill = factor(b_rep))) + 
  geom_bar(stat = "identity", position = "dodge") + 
  facet_grid(~ time)
...
gp1_plot <- ggplot(phagerna2, aes(x = strain, y = tpm, fill = factor(b_rep))) + 
  geom_bar(stat = "identity", position = "dodge") + 
  facet_grid(~ time)
...
gp1_10_plot <- plot_grid(gp10_plot, gp1_plot, ncol = 1, labels = c("A", "B"))
...
ribo_rna <- ggplot(phagerna %>% filter(gene_type == "tRNA"), aes(x = strain, y = counts)) + 
