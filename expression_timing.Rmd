...
ggplot(sim %>% filter(tabasco_id %in% genes, time <= 600), aes(x = time, y = sim_count, color = tabasco_id)) + 
  geom_line() + 
  coord_cartesian(xlim = c(200, 700), ylim = c(0, 3000)) + 
  geom_text_repel(
    data = filter(sim, tabasco_id %in% genes, time == 600),
...
ggplot(sim %>% filter(tabasco_id %in% prots, time <= 600), aes(x = time, y = sim_count, color = tabasco_id, group = tabasco_id)) + 
  geom_line() + 
  coord_cartesian(xlim = c(200, 700), ylim = c(0, 7500)) + 
  geom_text_repel(
    data = filter(sim, tabasco_id %in% prots, time == 600),
    aes(label = tabasco_id),
    size = 4,
    nudge_x = 6,
    fontface = "bold",
...
ggplot(sim %>% filter(tabasco_id %in% genes, time <= 600), aes(x = time, y = sim_count, color = tabasco_id, group = tabasco_id)) + 
  geom_line() + 
  coord_cartesian(xlim = c(200, 700), ylim = c(0, 3000)) + 
  geom_text_repel(
    data = filter(sim, tabasco_id %in% genes, time == 600),
...
ggplot(sim %>% filter(tabasco_id %in% prots, time <= 600), aes(x = time, y = sim_count, color = tabasco_id, group = tabasco_id)) + 
  geom_line() + 
  coord_cartesian(xlim = c(200, 700), ylim = c(0, 7000)) + 
  geom_text_repel(
    data = filter(sim, tabasco_id %in% prots, time == 600),
    aes(label = tabasco_id),
    size = 4,
