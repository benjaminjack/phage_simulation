...
all_data <- bind_rows(rep2, rep3, rep4, rep5) %>%
  mutate(strain = str_replace_all(strain, c("11-44" = "recoded", "11-42" = "evolved", "11-46" = "wildtype"))) %>%
...
all_data <- mutate(all_data, time = ifelse(b_rep == 1 & time == "1min" & strain == "wt", "temp", time)) %>% 
  ...
  # Correct replicate 4 wildtype labeling?
  # mutate(time = ifelse(b_rep == 4 & strain == "wt" & time == "5min", "temp", time)) %>%
  # mutate(time = ifelse(b_rep == 4 & strain == "wt" & time == "1min", "5min", time)) %>%
  # mutate(time = ifelse(b_rep == 4 & strain == "wt" & time == "temp", "1min", time))

# Join in standard gene labels
# This should probably take place in post_process.R from some master list of proteins that we're interested in
...
all_data <- filter(all_data, org == "phage") %>% 
  ...
  mutate(tabasco_id = str_replace_all(tabasco_id, c("gp10A" = "gp10")))
  # left_join(all_data, labels, by = c("Proteins" = "Accession")) %>% filter(org == "phage")
```

### Major capsid protein levels are depressed in attenuated and evolved strains

```{r}
all_data$time <- factor(all_data$time, labels = c("1min" = "1 minute", "5min" = "5 minutes", "9min" = "9 minutes"))
all_data$strain <- factor(all_data$strain,  levels = c("recoded", "evolved", "wildtype"))
maj_cap_plot <- ggplot(filter(all_data, tabasco_id == 'gp10'), 
                       aes(x = strain, y = area_norm)) + 
  geom_point(size = 2, color="grey85") +
  geom_line(aes(group=b_rep), color="grey85") +
  stat_summary(geom="errorbar", fun.y="mean", aes(ymin=..y.., ymax=..y.., group = strain), size=1) +
  facet_wrap(~ time, label = ) +
  panel_border() +
...
# prot_list <- c('gp10','gp11','gp12','gp13','gp14','gp15', 'gp18', 'gp3')

# prot_list <- c('gp10','gp11','gp12','gp13','gp14','gp15')

prot_list <- c('gp7','gp9')

class3_plot_data <- filter(all_data, 
                           tabasco_id %in% prot_list)

class3_plot_data$tabasco_id <- factor(class3_plot_data$tabasco_id, levels = prot_list)

class3_plot <- ggplot(class3_plot_data, 
                      aes(x = strain, y = area_norm, group=b_rep)) + 
  geom_point(size = 3, color="grey85") +
...
class3_plot_ms <- class3_plot %+% filter(class3_plot_data, time == "9 minutes") + 
  facet_wrap(~tabasco_id, scales = "free_y", ncol = 2, dir = "h")

class3_plot_beacon <- class3_plot %+% filter(class3_plot_data, time == "9 minutes") + 
  facet_wrap(~tabasco_id, scales = "free_y", nrow = 2, dir = "h")

class3_9_plot_beacon <- class3_plot %+% filter(class3_plot_data, time == "9 minutes") + 
  facet_wrap(~tabasco_id, scales = "free_y", ncol = 2, dir = "h")
...
save_plot("figures/class3_beacon.pdf", class3_plot_beacon, base_height = 5, base_width = 10)

save_plot("figures/class3_9_beacon.pdf", class3_9_plot_beacon, base_height = 4, base_width = 8)
...
make_abundance_plot <- function(protein_data) {
  ...
  myplot <- ggplot(sums %>% filter(time == "9 minutes", !is.na(class)), aes(x = strain, y = area_mean, group = strain)) + 
...
time_diff_data <- filter(all_data, 
                           tabasco_id %in% c('gp9', 'gp11'))

time_diff_plot <- ggplot(time_diff_data, 
...
save_plot("figures/time_diff.pdf", time_diff_plot, base_height = 5, base_width = 9)
...
ratio_data <- filter(all_data, 
                   tabasco_id %in% c('gp9', 'gp10A', 'gp17', 'gp8'), 
...
ratio_data2 <- filter(ratio_data, time == "9min") %>%
  ...
  bind_rows(data.frame(strain = "true_ratio", gp10A = 415, gp17=18, gp9=125, gp8=12)) %>%
  mutate(`gp10A:gp9` = gp10A/gp9,
         `gp10A:gp17` = gp10A/gp17,
         `gp10A:gp8` = gp10A/gp8,
         `gp9:gp17` = gp9/gp17,
         `gp9:gp8` = gp9/gp8) %>%
  select(-gp10A, -gp9, -gp8, -gp17)
