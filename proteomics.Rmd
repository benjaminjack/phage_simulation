...
maj_cap_plot <- ggplot(filter(all_data, tabasco_id == 'gp10'), 
  ...
  stat_summary(geom="bar", fun.y="mean", aes(group = strain), size=1, fill="grey50") +
  geom_point(size = 2) +
...
class3_plot <- ggplot(class3_plot_all_data, 
                      aes(x = strain, y = area_norm)) + 
  stat_summary(geom="bar", fun.y="mean", aes(group=strain), fill="grey50") +
  geom_point(size = 2) +
  geom_line(aes(group=b_rep)) +
...
time_diff_data <- filter(all_data, 
                           tabasco_id %in% c('gp9', 'gp11'), time == "9min")

# Put genes in numerical order
time_diff_data$tabasco_id <- factor(time_diff_data$tabasco_id, levels = c('gp9', 'gp11'))
...
time_diff_plot <- ggplot(time_diff_data, 
  ...
  facet_wrap(~tabasco_id, scales = "free_y") +
  scale_x_discrete(label = strain_labels) +
  panel_border() +
  ylab("relative protein abundance") +
  theme(legend.position = "none")
...
ratio_data2 <- filter(ratio_data, time == "9min") %>%
  ...
  bind_rows(data.frame(strain = "true_ratio", gp10 = 415, gp17=18, gp9=125, gp8=12)) %>%
