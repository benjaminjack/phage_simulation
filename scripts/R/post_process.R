...
rep1 <- combine_tech_reps(files) %>% mutate(strain = '11-46', time = 9, rep = 1)
...
rep2 <- combine_tech_reps(files) %>% mutate(strain = '11-46', time = 9, rep = 4)
rep2 <- normalize_areas(rep2)
write_csv(rep2, "/Volumes/Seagate/proteomics/post_processed/rep4.csv")

files <- c("/Volumes/Seagate/proteomics/11-44_9min_5a.msf", 
           "/Volumes/Seagate/proteomics/11-44_9min_5b.msf")
rep3 <- combine_tech_reps(files) %>% mutate(strain = '11-46', time = 9, rep = 5)
rep3 <- normalize_areas(rep3)
write_csv(rep3, "/Volumes/Seagate/proteomics/post_processed/rep5.csv")

all_strains <- bind_rows(rep1, rep2, rep3)
