...
all_data <- read_csv('./rep1234_combined_apex.csv', col_types = list(strain = col_character())) %>%
  filter(!is.na(class)) %>%
...
ggplot(filter(all_data, tabasco_id == 'gp10A', strain == "wt"), aes(x = factor(time), y = apex, group=b_rep, color=factor(b_rep))) + 
  geom_point(size = 3) +
  geom_line() + 
  panel_border() +
...
make_abundance_plot <- function(protein_data) {
  ...
  sums <- group_by(protein_data, strain, time, b_rep) %>% 
    summarize(apex_sum = sum(apex_norm)) %>% 
    filter(!is.na(apex_sum)) %>%
    group_by(strain, time) %>%
    summarize(apex_mean = mean(apex_sum), apex_sd = sd(apex_sum)) %>%
