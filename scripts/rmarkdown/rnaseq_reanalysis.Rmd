...
rep1 <- import_counts("../../data/PA36_recounted_q10_9min_rep1.tsv") %>% mutate(b_rep = 1, time = 9)
rep2 <- import_counts("../../data/PA45_recounted_q10_9min_rep2.tsv") %>% mutate(b_rep = 2, time = 9)
rep3 <- import_counts("../../data/PA54_recounted_q10_9min_rep3.tsv") %>% mutate(b_rep = 3, time = 9)

rep1_5 <- import_counts("../../data/PA35_recounted_q10_5min_rep1.tsv") %>% mutate(b_rep = 1, time = 5)
rep2_5 <- import_counts("../../data/PA44_recounted_q10_5min_rep2.tsv") %>% mutate(b_rep = 2, time = 5)
rep3_5 <- import_counts("../../data/PA53_recounted_q10_5min_rep3.tsv") %>% mutate(b_rep = 3, time = 5)

rep1_1 <- import_counts("../../data/PA34_recounted_q10_1min_rep1.tsv") %>% mutate(b_rep = 1, time = 1)
rep2_1 <- import_counts("../../data/PA43_recounted_q10_1min_rep2.tsv") %>% mutate(b_rep = 2, time = 1)
rep3_1 <- import_counts("../../data/PA52_recounted_q10_1min_rep3.tsv") %>% mutate(b_rep = 3, time = 1)

hisat <- import_counts("../../data/PA54_recounted_q10_9min_rep3.tsv") %>% mutate(b_rep = 3, time = 9)
# kallisto_recounted <- import_counts("/Volumes/Seagate/kallisto_pseudobam/PA54_test_counts.tsv") %>% mutate(b_rep = 1, time = 9)

# kallisto_plot <- ggplot(bind_rows(hisat, kallisto_recounted), aes(x = factor(gene_number, levels=unique(mixedsort(gene_number))), y = tpm, fill=factor(b_rep))) +
#   geom_bar(stat="identity", position = "dodge") +
#   theme(axis.text.x=element_text(angle=45,hjust=1), legend.position = "none") +
#   xlab("gene")

all_reps <- bind_rows(rep1, rep2, rep3, rep1_5, rep2_5, rep3_5, rep1_1, rep2_1, rep3_1) %>% mutate(strain = "wildtype")
...
plot_5 <- ggplot(all_reps %>% filter(time == 5, !is.na(class)), aes(x = factor(gene_number, levels=unique(mixedsort(gene_number))), y = tpm)) + 
  geom_bar(stat="summary", fun.y = "mean", aes(fill = factor(class))) +
...
sim_data_wt <- load_simulation("../../runs/012318/run_T7_012318b_wt")
