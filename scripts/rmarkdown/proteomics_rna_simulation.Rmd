...
if(SAVE_PLOTS) { save_plot("../figures/protein_rna_sim_scatter.pdf", protein_rna_sim_scatter, base_aspect_ratio = 3) } 
...
sim_data_cors <- filter(sim_data, proteins > 0, transcripts > 0) %>%
...
cor_plot <- ggplot(sim_data_cors, aes(x = time, y = cor, color = factor(weight), group = factor(weight))) + 
...
save_plot("../figures/cor_plot.pdf", cor_plot, base_aspect_ratio = 2.5)
